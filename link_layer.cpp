...
{
    if(send_queue_size >0 ) //contains packets
    {
        for(unsigned int i = 0;i<send_queue_size;i++) //iterate through the queue
        {
            Timed_packet p = send_queue[(end+i)%limit]; //set a packet p to the value from the send queue
            if(p.packet.header.seq > last_receive_ack-1)
            {
                send_queue_size -= (i+1);
                end += (i+1);
                break;
...
void Link_layer::send_timed_out_packets()
{
    ...
    for(unsigned int i = 0;i<send_queue_size;i++)
    {
        ...
        if(P.send_time < current)
        {
            P.packet.header.ack = next_receive_seq;
            P.packet.header.checksum = checksum(P.packet);
            if(physical_layer_interface->send((unsigned char*)&(P.packet),(P.packet.header.data_length + sizeof(P.packet.header))))
            {
                gettimeofday(&current,NULL);
